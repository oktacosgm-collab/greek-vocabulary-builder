...
# ── Sidebar ────────────────────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("## 🇬🇷 Greek Flashcards")
    enriched = sum(1 for w in word_list if w in cache)
    st.markdown(f"**{enriched:,} / {len(word_list):,}** words enriched")
    st.progress(enriched / max(len(word_list), 1))
    stats = SRS.get_stats(srs_data, [w for w in word_list if w in cache])
    st.markdown(f"**{stats['due']}** due · **{stats['new']}** new · **{stats['mature']}** mature")
    st.markdown("---")

    st.markdown("### Filters")
    diff_levels = ["A1","A2","B1","B2","C1","C2"]
    sel_diffs = st.multiselect("Difficulty", diff_levels, placeholder="All levels", label_visibility="collapsed")
    ...
    if chosen != "— select word —":
        ...
        if st.button("Go ➡️", use_container_width=True):
            ...
            if target in filtered:
                ...
                st.warning("Not in current filter. Set Difficulty to All.")
    st.markdown("---")

    c1, c2 = st.columns(2)
    c1.metric("✅ Known", st.session_state.score["known"])
    c2.metric("🔁 Review", st.session_state.score["review"])
    ...
    export_scope = st.radio("Export", ["All words","Current difficulty filter","Search results"], label_visibility="collapsed")
    if export_scope == "All words": export_words = word_list
    elif export_scope == "Current difficulty filter": export_words = filtered
    else: export_words = matches if matches else word_list
    st.markdown(f"*{len(export_words):,} words will be exported*")
    st.markdown("**Include fields:**")
    exp_pos         = st.checkbox("Part of speech & difficulty", value=True)
    exp_translation = st.checkbox("Translation", value=True)
    exp_definition  = st.checkbox("Definition", value=False)
    exp_example     = st.checkbox("Example sentence", value=True)
    export_opts = {"pos":exp_pos,"translation":exp_translation,"definition":exp_definition,"example":exp_example}
    if st.button("📄 Generate Word File", use_container_width=True, type="primary"):
        with st.spinner(f"Building Word doc for {len(export_words):,} words…"):
            buf = build_word_doc(export_words, all_words, export_opts)
        st.download_button(label="⬇️ Download .docx", data=buf,
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "🃏 Flashcard", "🧠 Spaced Repetition", "📚 Browse All",
    "📊 Conjugation & Declension", "🧪 Test Knowledge"
])
...
with tab1:
    ...
    st.markdown(f"<p class='progress-text'>Card {pos_raw+1} of {total} · {sel_diff}</p>", unsafe_allow_html=True)
    ...
    with b2:
        if st.button("🔊 Read Aloud", use_container_width=True, type="primary"): play_sequence(data,word)
    with b3:
        lbl="🔁 Flip Card" if not st.session_state.flipped else "⬆️ Reset Card"
        if st.button(lbl, use_container_width=True): st.session_state.flipped=not st.session_state.flipped; st.rerun()
    with b4:
        if st.session_state.flipped and st.button("✅ Known", use_container_width=True):
...
with tab2:
    ...
    s1.markdown(f"<div class='stat-box'><div class='stat-num' style='color:#f6ad55;'>{stats['due']}</div><div class='stat-label'>Due Today</div></div>", unsafe_allow_html=True)
    s2.markdown(f"<div class='stat-box'><div class='stat-num' style='color:#90cdf4;'>{stats['new']}</div><div class='stat-label'>New</div></div>", unsafe_allow_html=True)
    s3.markdown(f"<div class='stat-box'><div class='stat-num' style='color:#fbd38d;'>{stats['learning']}</div><div class='stat-label'>Learning</div></div>", unsafe_allow_html=True)
    s4.markdown(f"<div class='stat-box'><div class='stat-num' style='color:#68d391;'>{stats['mature']}</div><div class='stat-label'>Mature</div></div>", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    with col_due:
        if st.button(f"▶️ Review Due ({len(due_words)})", use_container_width=True, type="primary", disabled=len(due_words)==0):
            st.session_state.srs_queue=due_words.copy(); st.session_state.srs_index=0; st.session_state.srs_revealed=False; st.rerun()
    with col_new:
        if st.button(f"✨ Learn New ({min(20,len(new_words))})", use_container_width=True, disabled=len(new_words)==0):
    ...
    with col_all:
        if st.button("🔀 Review All Due + New", use_container_width=True, disabled=len(due_words)+len(new_words)==0):
    ...
    if not queue:
        if stats["due"]==0 and stats["new"]==0: st.success("🎉 Nothing due for review today! Come back tomorrow.")
        else: st.info("Choose a review mode above to start.")
    elif si>=len(queue):
        st.success(f"✅ Session complete! Reviewed {len(queue)} words.")
        if st.button("🔄 Start another session"): st.session_state.srs_queue=[]; st.session_state.srs_index=0; st.rerun()
    else:
        ...
        st.markdown(f"<p class='progress-text'>Card {si+1} of {len(queue)}</p>", unsafe_allow_html=True)
        st.progress(si/len(queue))
        reps=card.get("repetitions",0)
        if srs_word not in srs_data: srs_label="🆕 New"
        elif reps<3: srs_label=f"📖 Learning (×{reps})"
        else: srs_label=f"✅ Mature (×{reps})"
        ...
        if not revealed:
            ch=('<div class="card card-srs">'+f'<span class="pos-badge">{str_pos}</span>'+f'<span class="diff-badge">{str_dif}</span>'+f'<span class="srs-badge">{srs_label}</span>'+f'<div class="greek-word">{sw_safe}</div>'+f'<div class="transliteration">[ {str_safe} ]</div>'+(f'<hr class="example-divider"><div class="example-greek"><span style="font-size:0.8rem;opacity:0.6;">Παράδειγμα</span><br>{srs_ex}</div>' if srs_ex else '')+'<div class="flip-hint">↕ Reveal to rate your recall</div></div>')
            ...
            ch=('<div class="card card-srs">'+f'<span class="pos-badge">{str_pos}</span>'+f'<span class="diff-badge">{str_dif}</span>'+f'<span class="srs-badge">{srs_label}</span>'+f'{sgender_html}'+f'<div class="greek-word">{sw_safe}</div>'+f'<div class="transliteration">[ {str_safe} ]</div>'+f'<div class="translation">{srs_tr}</div>'+f'<div class="definition">{srs_df}</div>'+(f'<hr class="example-divider"><div class="example-greek"><span style="font-size:0.8rem;opacity:0.6;">Παράδειγμα</span><br>{srs_ex}</div><div class="example-en" style="display:block;margin-top:6px;">{srs_exen}</div>' if srs_ex else '')+'</div>')
            st.markdown(ch, unsafe_allow_html=True); st.markdown("<br>", unsafe_allow_html=True)
...
# ────────────────────────────────────────────────────────────────────────────────
with tab3:
    st.markdown("## 📚 Full Word List")
    search=st.text_input("🔍 Search", placeholder="Greek or English…")
    only_enriched=st.checkbox("Only enriched words")
    ...
    st.markdown(f"**{len(display):,}** words")
        ...
        with st.expander(f"**{w}** — {d.get('translation','--')}  {'✅' if w in cache else '⬜'}"):
            ...
            with c1:
                if w in cache:
                    ...
                    st.markdown(f"*{d.get('transliteration','--')}* · {d.get('part_of_speech','--')} · {d.get('difficulty','?')} · next review: **{due_str}**")
                    st.markdown(f"**Definition:** {d.get('definition','--')}"); st.markdown(f"**Example (GR):** *{d.get('example_greek','--')}*"); st.markdown(f"**Example (EN):** {d.get('example_english','--')}")
                else: st.info("Not yet enriched.")
            with c2:
                if st.button("🔊", key=f"b_{i}"): play_sequence(d,w)
    if len(display)>200: st.info(f"Showing 200 of {len(display)}. Use search to narrow down.")

# ────────────────────────────────────────────────────────────────────────────────
# TAB 4 — CONJUGATION & DECLENSION
# ────────────────────────────────────────────────────────────────────────────────
with tab4:
    st.markdown("## 📊 Conjugation & Declension")
    ...
    st.markdown(f"**{len(verbs)}** verbs with conjugation · **{len(nouns_adj)}** nouns/adjectives with declension"); st.markdown("---")
    col_s,col_p=st.columns([3,1])
    with col_s: t3_search=st.text_input("Search word", placeholder="Greek or English…", key="t3_search")
    with col_p: t3_pos=st.selectbox("Type",["All","Verbs only","Nouns/Adj only"],key="t3_pos")
    pool=verbs if t3_pos=="Verbs only" else nouns_adj if t3_pos=="Nouns/Adj only" else has_data
    if t3_search:
    ...
    for w in pool[:100]:
        ...
        with st.expander(f"**{w}** — {d.get('translation','--')}  ·  *{d.get('part_of_speech','')}*"):
            ...
            if d.get("conjugation"): st.markdown("#### Conjugation"); render_conjugation(w,d)
            if d.get("declension"): st.markdown("#### Declension"); render_declension(w,d)
    if len(pool)>100: st.info(f"Showing 100 of {len(pool)}. Use search to narrow down.")
...
with tab5:
    ...
    # ── Mode toggle ───────────────────────────────────────────────────────────
    mode = st.radio("Mode", ["🔤 Multiple Choice", "🎧 Listen & Type"],
                    horizontal=True, label_visibility="collapsed",
                    key="quiz_mode_radio")
    quiz_mode = "listen_type" if "Listen" in mode else "multiple_choice"
    ...
    sel_levels = st.multiselect("Difficulty levels", all_levels, default=all_levels,
    ...
    st.caption(f"{len(quiz_pool)} words available at: {level_str}")
    if len(quiz_pool) < 4:
        st.warning("Need at least 4 enriched words in selected levels to run quiz.")
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        with col_start:
            if st.button("▶️ Start New Quiz", use_container_width=True, type="primary"):
        ...
        if st.session_state.quiz_done:
            ...
            st.markdown(f"**{st.session_state.quiz_score['correct']} correct** out of {total_q} ({pct}%)")
            failed=st.session_state.quiz_failed
            if failed:
                st.markdown(f"**{len(failed)} words to review:**")
                for fw in failed: fd=cache.get(fw,{}); st.markdown(f"- **{fw}** — {fd.get('translation','--')}")
                st.markdown("---")
                if st.button("📄 Export failed words to Word", use_container_width=True):
                    ...
                    st.download_button(label="⬇️ Download failed_words.docx",data=buf,file_name="failed_words.docx",mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",use_container_width=True)
            else: st.success("🎉 Perfect score! No words to review.")
            ...
            st.markdown(f"<p class='progress-text'>Question {qi+1} of {len(qwords)}</p>", unsafe_allow_html=True); st.progress(qi/len(qwords))
            ...
            if answered is None:
                st.markdown("**Choose the correct translation:**"); cols=st.columns(2); labels=["A","B","C","D"]
                ...
                if answered==correct_tr: st.success(f"✅ Correct!  **{correct_tr}**  ·  *{qpos}*  ·  {qdif}")
                else: st.error(f"❌ Wrong.  You chose: *{answered}*"); st.markdown(f"Correct answer: **{correct_tr}**  ·  *{qpos}*  ·  {qdif}")
                st.markdown("<br>", unsafe_allow_html=True)
                ...
                with nb1:
                    if st.button("Next ➡️", use_container_width=True, type="primary"):
                        ...
                        st.rerun()
                with nb2:
                    if st.button("🚪 Exit Quiz", use_container_width=True): st.session_state.quiz_done=True; st.rerun()
                if st.session_state.quiz_failed:
                    with st.expander(f"📄 Export {len(st.session_state.quiz_failed)} failed words so far"):
                        if st.button("Generate Word file", key="mid_export", use_container_width=True):
                            with st.spinner("Building doc…"):
        ...
        else: st.info("Press **▶️ Start New Quiz** to begin.")
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        with cfg_col:
            lt_target = st.radio("Listen to", ["Word", "Example sentence"],
                                 horizontal=True, key="lt_target_radio")
            target_key = "word" if lt_target == "Word" else "example"
        with score_col:
        ...
        if st.button("▶️ Start Listen & Type Quiz", use_container_width=True, type="primary"):
        ...
        if st.session_state.lt_done:
            ...
            st.markdown("### 🏁 Listen & Type Complete!")
            st.markdown(f"**{st.session_state.lt_score['correct']} correct** out of {lt_total} ({lt_pct}%)")
            ...
            if failed:
                ...
                if st.button("📄 Export failed words to Word", use_container_width=True, key="lt_export"):
                    with st.spinner("Building Word doc…"):
                        buf = build_word_doc(failed, all_words,
                                             {"pos":True,"translation":True,"definition":True,"example":True})
                    st.download_button(label="⬇️ Download failed_words.docx", data=buf,
        ...
        # ── Active quiz ───────────────────────────────────────────────────────
            ...
            # Audio card
            target_label = "word" if ltgt == "word" else "example sentence"
            st.markdown(f"<div class='card' style='min-height:160px;'>"
                        f"<div style='color:#a0aec0;font-size:0.9rem;margin-bottom:12px;'>🎧 Listen and type the {target_label}</div>"
                        f"<div style='color:#718096;font-size:0.8rem;'>Press play, then type what you hear</div>"
            ...
            if not submitted:
                ...
                if result == "correct":
                    st.success(f"✅ {msg}  ·  **{correct_text}**")
                elif result == "close":
                    st.warning(f"〰️ {msg}  ·  You typed: *{user_ans}*  ·  Correct: **{correct_text}**")
                else:
                    st.error(f"❌ Incorrect  ·  You typed: *{user_ans}*")
                ...
                nb1, nb2 = st.columns([2,1])
                with nb1:
                    if st.button("Next ➡️", use_container_width=True, type="primary", key="lt_next"):
                        ...
                        st.rerun()
                with nb2:
                    if st.button("🚪 Exit Quiz", use_container_width=True, key="lt_exit"):
                        st.session_state.lt_done = True; st.rerun()
        else:
            st.info("Press **▶️ Start Listen & Type Quiz** to begin.")

st.markdown("---")
st.markdown("<p style='text-align:center;color:#4a5568;font-size:0.8rem;'>🇬🇷 Greek Flashcards · Streamlit · Claude API</p>", unsafe_allow_html=True)